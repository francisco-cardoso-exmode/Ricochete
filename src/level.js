...
b('base',0,-.35,5.5,10.6,.7,11.5,{color:0x707a72,shell:true});
...
b('upper',0,5.55,-.34,10.6,11.8,.65,{color:0x858d83,shell:true});
b('upper',-5.15,5.5,1.12,.35,11.7,2.8,{color:0x90998c,shell:true});
b('upper',5.15,5.5,1.12,.35,11.7,2.8,{color:0x90998c,shell:true});
b('upper',0,11.3,1.12,10.6,.35,2.8,{color:0x747f70,shell:true});
// Lower construction: staggered stacks, arches, crates, ramps and spring bumpers.
for (const x of [-3.95,3.95]) for(let row=0;row<3;row++) {
  b('base',x,.33+row*.64,3.8,.95,.59,1.0,{color:row%2?0xb4b8ad:0x8a9487});
  b('base',x,.33+row*.64,5.0,.95,.59,1.0,{color:row%2?0x8c9688:0xb8bcb0});
}
for(const x of [-2.85,2.85]) {
  b('base',x,.65,6.5,.55,1.3,.65,{color:0xc2c6b9});
  b('base',x,.65,8.05,.55,1.3,.65,{color:0xc2c6b9});
  add('base','arch',[x,1.3,7.28],[.75,.85,1.45],{axis:'z',color:0xb2b9ab});
}
b('base',-4,.46,9.5,1.25,.9,1.15,{crate:true,dynamic:true,color:0xb7bead});
b('base',3.7,.47,9.7,1.1,.9,1.05,{crate:true,dynamic:true,color:0xa7af9c});
b('base',3.65,1.4,9.7,.8,.9,.8,{crate:true,dynamic:true,color:0xd0d2c2});
add('base','bumper',[-2,.45,4.6],[.56,.45],{color:0xc6cbb9,bonus:'bumper'});
add('base','bumper',[2,.45,6.5],[.55,.45],{color:0xc0c8b2,bonus:'bumper'});
b('base',-3.6,.27,1.5,1.9,.45,2,{rotation:[-.18,0,0],color:0xb4bca9});
b('base',3.6,.32,1.5,1.9,.5,2,{rotation:[-.27,0,0],color:0xb4bca9});
...
for(const [x,y,z,w] of [[-3.55,2.6,1.3,2.2],[3.6,3.5,1,2.2],[-3.6,6,1.2,2.2],[3,7.5,.9,3.4],[-2,10,1.05,3.8]]) {
 b('upper',x,y,z,w,.42,1.9,{color:0x707e6b});
 b('upper',x,y+.27,z+.25,w-.15,.12,1.3,{color:0xc5cbb9});
}
...
b('upper',-2.65,4.1,.8,1.15,1.65,1.1,{crate:true,color:0x8f9d87});
